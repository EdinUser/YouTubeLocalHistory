// Simple storage wrapper using browser.storage.local instead of IndexedDB
    ...
    class SimpleStorage {
        ...
        async ensureMigrated() {
            ...
            try {
                ...
                const result = await browser.storage.local.get(['__migrated__']);
                ...
                // Mark as migrated
                await browser.storage.local.set({ '__migrated__': true });
                this.migrated = true;
        ...
        // Migrate data from IndexedDB to browser.storage.local
            ...
            return new Promise((resolve, reject) => {
                ...
                request.onsuccess = async (event) => {
                    ...
                    try {
                        ...
                        // Save all migrated data to browser.storage.local
        ...
        async getVideo(videoId) {
            ...
            const result = await browser.storage.local.get([`video_${videoId}`]);
        ...
        async setVideo(videoId, data) {
            await this.ensureMigrated();
            await browser.storage.local.set({ [`video_${videoId}`]: data });
        }

        // Remove video record
        async removeVideo(videoId) {
            await this.ensureMigrated();
            await browser.storage.local.remove([`video_${videoId}`]);
        }
        ...
        async getPlaylist(playlistId) {
            await this.ensureMigrated();
            const result = await browser.storage.local.get([`playlist_${playlistId}`]);
        ...
        async setPlaylist(playlistId, data) {
            await this.ensureMigrated();
            await browser.storage.local.set({ [`playlist_${playlistId}`]: data });
        }

        // Remove playlist record
        async removePlaylist(playlistId) {
            await this.ensureMigrated();
            await browser.storage.local.remove([`playlist_${playlistId}`]);
        }
